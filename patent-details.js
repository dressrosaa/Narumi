...
function showPatentModal(patentNumber) {
  ...
  let imageHtml = '';
  if (patent.image) {
    imageHtml = `<img src="${patent.image}" alt="${patent.title}" class="patent-modal-image">`;
  }

  // 2020年奨励賞の画像をセンタリング
  let centeredImageHtml = '';
  if (patent.additionalImages && patent.additionalImages.length > 0) {
    centeredImageHtml = `<div class="centered-image"><img src="${patent.additionalImages[0]}" alt="${patent.title}" class="patent-modal-additional-image"></div>`;
  }

  // 他の画像をfeaturesの下に配置
  let additionalImagesHtml = '';
  if (patent.additionalImages && patent.additionalImages.length > 1) {
